    ...
    <div className={`min-h-screen transition-colors ${dark ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <nav className={`sticky top-0 z-50 px-4 py-3 flex items-center justify-between border-b ${dark ? 'bg-gray-800/95 border-gray-700 backdrop-blur' : 'bg-white/95 border-gray-200 backdrop-blur'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { if (installPrompt) { installPrompt.prompt(); } else if ((window.navigator as any).standalone || window.matchMedia('(display-mode: standalone)').matches) { alert('✅ App is already installed! Check your home screen or app drawer.'); } else if (/Chrome/.test(navigator.userAgent) && !/Edge/.test(navigator.userAgent)) { alert('To install this app:\n\n👉 Click the install icon (⊕) in the address bar at the top\n\nOR\n\n👉 Click ⋮ menu → "Install FitZone Gym..."'); } else if (/iPhone|iPad/.test(navigator.userAgent)) { alert('To install on iPhone:\n\n1. Tap the Share button (□↑) at the bottom\n2. Tap "Add to Home Screen"\n3. Tap "Add"'); } else { alert('To install:\n\n👉 Look for the install icon (⊕) in your browser\'s address bar\n\nOR open ⋮ menu → "Install App"'); } }} className="px-3 py-1.5 bg-blue-500 text-white text-xs rounded-lg font-medium hover:bg-blue-600">📲 Install App</button>
